...
def train(args, train_loader_syn, train_loader_ugc,
    ...
    for step, ((syn_clip_i,syn_i1, syn_i2, dist_label_syn,y_i), (ugc_clip_j,ugc_i1, ugc_i2,_ ,y_j)) in \
        ...
        if args.nr == 0 and step % 5 == 0:
            ...
            et = time.time()
            print(f"Step [{step}/{args.steps}]\t Loss: {loss.item()}\t LR: {round(lr, 5)} Time: {et - st}")
            st = time.time()
...
def main(gpu, args):
    ...
    train_dataset_syn = image_caption_data(file_path=args.csv_file_syn, tokenizer=tokenizer,
                                           img_dir=r'F://syn', data_type='syn',
    ...
    indices_syn = random.sample(range(len(train_dataset_syn)), 200000)
    sampler_syn = SubsetRandomSampler(indices_syn)
    ...
    train_dataset_ugc = image_caption_data(file_path=args.csv_file_ugc, tokenizer=tokenizer,
                                           img_dir=r'F://UGC_data', data_type='ugc'
                                ,image_size=args.image_size)
    ...
    indices_ugc = random.sample(range(len(train_dataset_ugc)), 200000)
    sampler_ugc = SubsetRandomSampler(indices_ugc)

    train_loader_ugc = DataLoader(
        train_dataset_ugc,
        batch_size=args.batch_size,
